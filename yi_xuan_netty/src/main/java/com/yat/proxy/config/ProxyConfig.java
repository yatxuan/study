...
public class ProxyConfig implements Serializable {
     ...
     */
    private static volatile Map<String, List<Integer>> clientInetPortMapping = new HashMap<>(16);

    /**
     * 代理服务器上的每个对外端口（key）对应的代理客户端背后的真实服务器信息（value）
     */
    private static volatile Map<Integer, String> inetPortLanInfoMapping = new HashMap<>(16);

    /**
     * 配置变化监听器
     */
    private List<ConfigChangedListener> configChangedListeners = new ArrayList<>(16);
    ...
    public ProxyConfig(NettyProxyServerProperties nettyProxyProperties) {
        Map<String, List<Integer>> clientInetPortMapping = new HashMap<>(16);
        Map<Integer, String> inetPortLanInfoMapping = new HashMap<>(16);
        if (nettyProxyProperties.getClientServer() != null && nettyProxyProperties.getClientServer().size() > 0) {
            ...
            for (NettyProxyServerProperties.ClientServer clientSever : nettyProxyProperties.getClientServer()) {
              ...
              /*  for (ClientProxyMapping mapping : mappings) {
                ...
                ProxyConfig.clientInetPortMapping = clientInetPortMapping;
                ProxyConfig.inetPortLanInfoMapping = inetPortLanInfoMapping;
            }
